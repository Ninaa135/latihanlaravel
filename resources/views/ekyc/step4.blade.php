        ...
        <form method="POST" action="{{ route('ekyc.step4.store') }}">
            ...
            <div class="mb-4">
                ...
                <select name="referensi" class="mt-1 block w-full border-gray-300 rounded-md">
                    ...
                    <option value="Teman" {{ old('referensi', $data->referensi ?? '') == 'Teman' ? 'selected' : '' }}>Teman</option>
                    <option value="Langsung dari Kampus" {{ old('referensi', $data->referensi ?? '') == 'Langsung dari Kampus' ? 'selected' : '' }}>Langsung dari Kampus</option>
                </select>
            </div>

            {{-- Navigasi --}}
            <div class="flex justify-between items-center mt-4">
                <a href="{{ route('ekyc.step3') }}" class="text-sm text-gray-500 hover:text-gray-700">
                    ← Kembali ke Step 3
                </a>
